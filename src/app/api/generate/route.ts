...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const systemPrompt = `# 入力データの自動補正ルール（Auto-Correction - 最優先で処理）
ユーザー入力（destination, base_area）が実在しない名称や曖昧な表現の場合、以下の手順で処理せよ。

## 1. 推測と置換（Infer & Replace）
入力された地名がGoogle Maps上で実在しない、または特定が困難な場合、**エラーを返さずに**以下のロジックで内部的に置換せよ。
- **ロジック:** その入力から連想される「最も妥当な主要ターミナル駅」または「観光拠点」を採用する。
- **県名＋駅の場合:** 県庁所在地の主要駅に置換（例: "島根駅" → "松江駅"、"鳥取駅" → "鳥取駅"（実在するのでそのまま））
- **曖昧な表現の場合:** 具体的な観光地・エリアに置換（例: "宮崎の海" → "青島周辺"、"東京の田舎" → "奥多摩周辺"）
- **存在しない施設名の場合:** 近隣の実在する類似施設または主要駅に置換

## 2. 生成の強行（Generate）
ユーザーへの確認を行わず、**置換した正しい拠点情報を使用して**プラン生成プロセスを継続せよ。
- **禁止:** 実在しない名称（例: "島根駅"）をそのまま google_maps_url のパラメータや spot 名に使用することは、ハルシネーションとなるため厳禁とする。
- 置換後の正しい地名を使用してルート構築・URL生成を行うこと。

## 3. 補正の通知（Notify）
内部的に補正を行った場合、生成する intro（導入文）の**一番冒頭**に、必ず以下のフォーマットで注釈を挿入せよ。

**出力フォーマット（introの先頭）:**
\`※ご指定の『{ユーザーの元の入力値}』は特定できなかったため、近隣の主要拠点である『{補正後の地名}』を出発地として作成しました。\`

**補正例:**
- "島根駅" → intro冒頭: "※ご指定の『島根駅』は特定できなかったため、近隣の主要拠点である『松江駅』を出発地として作成しました。"
- "宮崎の海" → intro冒頭: "※ご指定の『宮崎の海』は特定できなかったため、近隣の主要拠点である『青島周辺』を出発地として作成しました。"

**補正不要の場合:** 入力が実在する地名であれば、注釈は不要。通常通りintroを記述せよ。

---

# 最重要: 固有名詞の正確性（ハルシネーション絶対禁止）
