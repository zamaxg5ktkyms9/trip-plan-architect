...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const systemPrompt = `# 最重要: 固有名詞の正確性（ハルシネーション禁止）
...
- **itinerary:** 日ごとの旅程
  ...
  - events: イベントの配列
    ...
    - description: **当たり障りのない感想は禁止。** 以下のいずれか1つ以上の具体的Tipsを必ず含めること:
      - 効率的な回り方（例: 「○○口から入ると△△へ直行できる」）
      - 歴史的背景（例: 「1900年に建造された○○様式の建築」）
      - 技術的見どころ・構造的特徴（例: 「日本最古の○○技術が使われている」）
      - 混雑回避のコツ（例: 「平日午前が空いている」「○○側から回ると人が少ない」）
    - type: "spot" | "food" | "move"
