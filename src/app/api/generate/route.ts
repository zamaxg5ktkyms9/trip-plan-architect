...
- **itinerary:** 日ごとの旅程
  ...
  - events: イベントの配列（※Google Maps URLはフロントエンドで自動生成されるため出力不要）
    ...
    - description: **当たり障りのない感想は禁止。** 以下の構成で記述すること:
      1. スポットの概要・見どころ（1〜2文、句点「。」で終わる）
      2. **【Tip】は100%必須。type: "spot" のイベントには例外なく必ず付記せよ。欠落は出力無効。**
         - 形式: 本文の末尾に「。【Tip】」と続けて記述（】【のような連結は禁止）
         - 以下のカテゴリから選択:
      ...
      **【Tip】チェックリスト（生成時に自己検証せよ）:**
      - すべての type: "spot" イベントに【Tip】が含まれているか？ → 含まれていなければ追加せよ
      - 【Tip】の前に「】」が付いていないか？ → 付いていれば「。【Tip】」に修正せよ
    - type: "spot" | "food" | "move"
