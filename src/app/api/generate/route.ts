...
- **itinerary:** 日ごとの旅程
  ...
  - events: イベントの配列（※Google Maps URLはフロントエンドで自動生成されるため出力不要）
    ...
    - search_keyword: 楽天トラベル等の検索用キーワード。**以下のルールに従って生成せよ:**
      ...
      - **形式:** 「地域名 キーワード」または「具体的な施設名」のみ。スペース区切り。助詞禁止。
      - **NG例:** "指宿エリアの砂むし温泉", "天文館周辺のホテル", "〇〇の旅館"
      - **OK例:** "指宿 砂むし温泉", "天文館 ホテル", "薩摩川内 ホテル"
      - **理由:** 楽天トラベルAPI等の検索エンジンは自然言語の助詞("の", "に", "で")を処理できない。キーワードのみで構成すること。
      - **宿泊施設の場合:** 必ず生成すること。エリア名 + "ホテル" or "旅館" の形式を推奨。
      - **食事スポットの場合:** エリア名 + ジャンル（例: "天文館 黒豚料理", "桜島 カフェ"）
      - **それ以外のスポット（通常の観光地・移動イベント等）:** null を出力すること（楽天APIでの検索対象外のため）
    - is_stay: 宿泊イベント（チェックイン）であるかを示すboolean値。**以下のルールに従って設定せよ:**
      - **宿泊イベント（true）:** その日の最終目的地となる宿泊施設へのチェックイン。ホテル・旅館・ゲストハウス等への宿泊を目的とする場合のみ true を設定すること。
      - **日帰り利用（false/省略）:** 昼間の温泉入浴、レストランでの食事、施設見学など、宿泊を伴わない利用の場合は false を設定するか、省略すること。
      - **例（true）:** 「指宿温泉の旅館にチェックイン」「鹿児島市内のホテルで宿泊」
      - **例（false）:** 「指宿白水館で日帰り温泉」「ホテルレストランでランチ」
