...
/**
 * Schema for a single event in the itinerary (Short-key Object format for token efficiency + OpenAI compatibility)
 * Short keys reduce token usage while maintaining OpenAI Structured Outputs compatibility
 *
 * Key mapping:
 * - t: time (string) - Time of the event (e.g., "09:00")
 * - n: name (string) - Name of the place or activity
 * - a: activity (string) - Description of the activity
 * - tp: type (EventType) - Type of activity (spot/food/work/move)
 * - nt: note (string) - Additional notes or details
 * - q: imageSearchQuery (string | null) - English search query for Unsplash API
 */
